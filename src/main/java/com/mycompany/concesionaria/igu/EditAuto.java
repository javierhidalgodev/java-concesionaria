 ...
 * @author Javi
 */
public class EditAuto extends javax.swing.JFrame {

    Controladora controladora;
    DataView parentFrame;
    Automovil auto;

    public EditAuto(Controladora controladora, DataView parentFrame, Automovil auto) {
        initComponents();
    ...
    private void editAuto() {
        Automovil formData = getFormData();

        if (dataChanges()) {
            int confirmation = NotificationHandle.showConfirmationDialog(pPrincipal, Message.CONFIRMAR_EDITAR);

            if (confirmation == 0) {
                auto.setBrand(formData.getBrand());
                auto.setModel(formData.getModel());
                auto.setColor(formData.getColor());

                try {
                    controladora.editAutomovil(auto);
                    parentFrame.initTable();

                    NotificationHandle.showInfoDialog(pPrincipal, Message.COCHE_EDITADO);
                    dispose();
                } catch (Exception e) {
            ...
            NotificationHandle.showWarningDialog(pPrincipal, Message.MISMOS_DATOS);
        }
    }

    private Automovil getFormData() {
        String marca = txtMarca.getText().trim();
        String modelo = txtModelo.getText().trim();
        String color = txtColor.getText().trim();
        String placa = txtPlaca.getText().trim();

        Automovil autoData = new Automovil(marca, modelo, color, placa);

        return autoData;
    }

    private boolean isFormValid() {
        Automovil autoData = getFormData();

        return !autoData.getBrand().isEmpty() && !autoData.getModel().isEmpty() && !autoData.getColor().isEmpty() && !autoData.getPlate().isEmpty();
    }

    private boolean dataChanges() {
        Automovil formData = getFormData();

        return !auto.getBrand().equals(formData.getBrand()) || !auto.getModel().equals(formData.getModel()) || !auto.getColor().equals(formData.getColor());
    }
